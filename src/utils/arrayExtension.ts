...
declare global {
  interface Array<T> {
    ...
    firstOrDefault(filter?: (item: T) => any): T | null;
    lastOrDefault(filter?: (item: T) => any): T | null;
...
Array.prototype.groupBy = function <T, U extends keyof any>(
  func: (item: T) => any
) {
  let gs = this.reduce(
...
Array.prototype.orderByText = function <T>(
  selector: (item: T) => any
): Array<T> {
  let desc = false;
  let res = [...this].sort((a, b) => {
